...
# dongyizhe
"""
IM test with optional --weights-tag (must match main_immunogenicity_train_dongyizhe_20260405.py).
Official upstream: main_immunogenicity_test.py (no --weights-tag).
"""
import os
...
@click.option(
    '--weights-tag',
    default=None,
    help='Must match --weights-tag used in main_immunogenicity_train_dongyizhe_20260405.py.',
)
def main(data_cnf, model_cnf, start_id, num_models, weights_tag):
    ...
    imm_model_path = Path(os.path.join(Path(model_cnf['path']), 'IM', f'{out_name}.pt'))
